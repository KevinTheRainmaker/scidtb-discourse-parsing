"""
Fine-tuned model discourse parsing experiment.

This script:
1. Loads test data
2. Parses using fine-tuned model
...
def parse_test_set(
    ...
    """
    Parse test set using fine-tuned parser.
    
    Args:
        parser: FineTunedParser instance
    ...
    logger.info(f"Parsing {len(gold_trees)} test samples with fine-tuned model...")
...
def run_experiment(
    ...
    """
    Run complete fine-tuned model experiment.
    ...
    """
    logger.info("="*70)
    logger.info("Fine-tuned Model Discourse Parsing Experiment")
    logger.info("="*70)
    ...
    # Initialize parser
    logger.info("\n[2] Initializing fine-tuned parser...")
    parser = FineTunedParser(
    ...
    if valid_predictions:
        ...
        # Print results
        metrics.print_summary(title="Fine-tuned Model Results")
    ...
    if save_predictions_flag:
        pred_file = output_dir / f"finetuned_predictions_{timestamp}.json"
    ...
    if metrics:
        ...
        results_file = output_dir / f"finetuned_results_{timestamp}.json"
    ...
    # Final summary
    print(f"\n{'='*70}")
    print(f"{'Fine-tuned Model Experiment Complete':^70}")
    print(f"{'='*70}")
...
def compare_with_baselines(
    finetuned_results_file: Path,
    baseline_files: List[Path],
    output_dir: Path
):
    """
    Compare fine-tuned results with baseline approaches.
    
    Args:
        finetuned_results_file: Path to fine-tuned results JSON
        baseline_files: List of paths to baseline results (zero-shot, few-shot)
    ...
    for baseline_file in baseline_files:
        if baseline_file.exists():
            with open(baseline_file, 'r') as f:
                ...
                name = baseline['experiment'].replace('_', '-').title()
                comparison[name] = baseline['summary']
    
    # Print comparison table
    print(f"\n{'='*80}")
    print(f"{'Model Comparison':^80}")
    print(f"{'='*80}")
    print(f"{'Approach':>20} | {'UAS':>10} | {'LAS':>10} | {'F1':>10} | {'Samples':>8}")
    print(f"{'-'*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run fine-tuned model discourse parsing experiment"
    )
    ...
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('./data/results/finetuned'),
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Fine-tuned model ID (e.g., ft:gpt-3.5-turbo:...)'
    )
    parser.add_argument(
        '--temperature',
        type=float,
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs='+',
        help='Baseline result files for comparison'
    )
    ...
    try:
        ...
        if args.compare and args.baseline_files:
            # Find the most recent finetuned results file
            result_files = sorted(args.output_dir.glob("finetuned_results_*.json"))
            if result_files:
